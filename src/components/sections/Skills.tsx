      ...
      <div className="mx-auto max-w-[var(--container-width)] px-6">
        ...
        <StaggerGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {skills.map((group) => (
            <StaggerItem
              key={group.category}
              className="rounded-[var(--radius-md)] border border-border bg-surface p-6 backdrop-blur-sm transition-shadow duration-300 hover:border-accent-2 hover:shadow-[var(--glow-accent-2)]"
            >
